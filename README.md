...
- **Save contact** links to `/card/johnson-jiang.vcf`, a real `text/vcard`
  ...
  download a named `.vcf`. The page appends `?met=YYYY-MM-DD` (the
  recipient's local date) so the contact note records the day you met, and
  the contact photo is the mark embedded in the card page itself.
